# AI视频助手 (AI Video Assistant)

[![Python](https://img.shields.io/badge/Python-3.10+-blue.svg)](https://python.org)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Status](https://img.shields.io/badge/Status-Active-brightgreen.svg)]()

基于多模态技术的智能视频内容分析系统，支持视频转写、文本处理、语义检索和智能问答。

## 🌟 项目特色

- **🎥 全流程视频处理**：从视频文件到结构化文本的完整流水线
- **🔍 双重检索系统**：向量语义检索 + BM25关键词检索
- **🌐 多语言支持**：支持99种语言的语音识别和文本翻译
- **📝 智能文本处理**：自动清洗、分段、翻译转写结果
- **⚡ 高性能检索**：毫秒级响应，支持大规模视频库检索
- **🔧 模块化设计**：易于扩展和定制

## 🚀 快速开始

### 环境要求

- Python 3.10+
- FFmpeg (用于音视频处理)
- 可选：CUDA支持的GPU (用于加速)

### 安装步骤

1. **克隆项目**
```bash
git clone https://github.com/1625353276/Video_Assistant.git
cd Video_Assistant
```

2. **创建虚拟环境**
```bash
# 使用conda (推荐)
conda create -n video_assistant python=3.10
conda activate video_assistant

# 或使用venv
python -m venv video_assistant
source video_assistant/bin/activate  # Linux/Mac
video_assistant\Scripts\activate     # Windows
```

3. **安装依赖**
```bash
pip install -r requirements.txt
```

4. **安装系统依赖**
```bash
# Windows: 下载FFmpeg并添加到PATH
# https://ffmpeg.org/download.html

# macOS
brew install ffmpeg

# Ubuntu/Debian
sudo apt update
sudo apt install ffmpeg
```

5. **可选：安装分词库（提升检索效果）**
```bash
# 中文分词
pip install jieba

# 英文分词
pip install nltk
python -c "import nltk; nltk.download('punkt'); nltk.download('stopwords')"
```

### 基本使用

1. **视频转写**
```bash
# 基础转写
python main.py --video data/raw_videos/your_video.mp4

# 指定模型和输出目录
python main.py --video data/raw_videos/your_video.mp4 --model small --output ./output
```

2. **文本处理**
```python
from modules.text.text_cleaner import TextCleaner
from modules.text.segmenter import TextSegmenter
from modules.text.translator import TextTranslator

# 文本清洗
cleaner = TextCleaner()
cleaned_text = cleaner.clean_text("嗯 这个 就是 今天的 内容")

# 文本分段
segmenter = TextSegmenter(max_tokens=400)
segments = segmenter.segment_by_sentences(long_text)

# 文本翻译
translator = TextTranslator()
result = translator.translate("深度学习", target_lang="en")
```

3. **内容检索**
```python
from modules.retrieval.vector_store import VectorStore
from modules.retrieval.bm25_retriever import BM25Retriever

# 向量语义检索
vector_store = VectorStore()
vector_store.add_documents(documents)
results = vector_store.search("智能手机定位原理", top_k=5)

# BM25关键词检索
bm25 = BM25Retriever()
bm25.add_documents(documents)
results = bm25.search("GPS 卫星", top_k=5)
```

## 📁 项目结构

```
Video_Assistant/
├── 📄 main.py                 # 主程序入口
├── 📄 test_pipeline.py        # 流水线测试
├── 📄 requirements.txt        # 依赖包列表
├── 📁 modules/                # 核心功能模块
│   ├── 📁 video/             # 视频处理 ✅
│   │   ├── video_loader.py
│   │   └── audio_extractor.py
│   ├── 📁 speech/            # 语音识别 ✅
│   │   └── whisper_asr.py
│   ├── 📁 text/              # 文本处理 ✅
│   │   ├── text_cleaner.py   # 文本清洗
│   │   ├── segmenter.py      # 文本分段
│   │   └── translator.py     # 文本翻译
│   ├── 📁 retrieval/         # 检索系统
│   │   ├── vector_store.py   # 向量存储 ✅
│   │   ├── bm25_retriever.py # BM25检索 ✅
│   │   ├── hybrid_retriever.py # 混合检索 ⏳
│   │   └── multi_query.py    # 多查询生成 ✅
│   ├── 📁 qa/                # 问答系统 ⏳
│   └── 📁 utils/             # 工具模块 ✅
├── 📁 data/                  # 数据目录
│   ├── 📁 raw_videos/        # 原始视频
│   └── 📁 transcripts/       # 转写结果
├── 📁 models/                # 模型缓存
├── 📁 tests/                 # 测试文件
└── 📁 deploy/                # 部署相关 ⏳
```

## 🎯 核心功能

### 视频处理流水线
```
视频文件 → 视频校验 → 音频提取 → 语音识别 → 多格式输出
```

### 文本处理流水线
```
原始文本 → 去除填充词 → 文本分段 → 多语言翻译 → 结果保存
```

### 智能检索系统
```
用户查询
    ↓
多查询生成 ✅
    ↓ (扩展多样化查询)
┌─────────────────┬─────────────────┐
│   向量检索      │    BM25检索     │
│ (语义相似度)    │  (关键词匹配)   │
└─────────────────┴─────────────────┘
    ↓
结果融合排序
    ↓
精准检索结果
```

### 多查询生成器
- **规则扩展**: 同义词替换、领域术语映射、句式变化
- **模型扩展**: 语义相似查询、关键词扩展
- **智能权重**: 自动分配和归一化查询权重
- **多语言支持**: 中英文混合查询处理

## 📊 性能指标

| 功能模块 | 处理速度 | 准确性 | 适用场景 |
|---------|---------|--------|---------|
| Whisper语音识别 | 0.5x-2x实时 | 95%+ | 多语言转写 |
| 向量检索 | ~6ms | 语义匹配 | 概念查询 |
| BM25检索 | ~1ms | 精确匹配 | 关键词查询 |
| 文本清洗 | ~10ms | - | 预处理 |

## 🛠️ 模型配置

### Whisper语音识别模型
| 模型 | 大小 | 速度 | 准确性 | 推荐场景 |
|------|------|------|--------|---------|
| tiny | 75MB | 最快 | 基础 | 快速测试 |
| base | 142MB | 快速 | 良好 | 日常使用 |
| small | 466MB | 中等 | 较好 | 平衡选择 |
| medium | 1.5GB | 较慢 | 很好 | 高质量需求 |
| large | 2.9GB | 最慢 | 最佳 | 专业用途 |

### 向量化模型
- **默认**: sentence-transformers/all-MiniLM-L6-v2
- **维度**: 384维
- **大小**: 90MB
- **特点**: 轻量级，多语言支持

## 📝 输出格式

### JSON格式（推荐）
```json
{
  "audio_file": "video.mp4",
  "language": "zh",
  "text": "完整转写文本...",
  "segments": [
    {
      "id": 0,
      "start": 0.0,
      "end": 5.2,
      "text": "第一段文本",
      "confidence": 0.95
    }
  ],
  "model_used": "base",
  "total_duration": 120.5,
  "avg_confidence": 0.92
}
```

### 支持格式
- **JSON**: 结构化数据，包含时间戳和置信度
- **TXT**: 纯文本格式，易于阅读
- **SRT**: 标准字幕格式，支持视频播放器
- **VTT**: Web视频字幕格式

## 🧪 测试

### 运行测试
```bash
# 基础流水线测试
python test_pipeline.py

# 向量存储测试
python tests/test_vector_store.py

# BM25检索器测试
python tests/test_bm25_retriever.py

# 多查询生成器测试
python tests/test_multi_query.py

# 检索系统集成测试
python tests/test_retrieval_integration.py
```

### 测试覆盖
- ✅ 视频处理和音频提取
- ✅ Whisper语音识别
- ✅ 文本清洗和分段
- ✅ 文本翻译
- ✅ 向量存储和检索
- ✅ BM25关键词检索
- ✅ 索引持久化

## 🔄 开发状态

### ✅ 已完成
- **视频处理模块**: 完整实现并测试
- **语音识别模块**: 支持多种Whisper模型
- **文本处理模块**: 清洗、分段、翻译功能
- **向量存储模块**: 语义检索和索引管理
- **BM25检索模块**: 关键词匹配和评分
- **多查询生成器**: 查询扩展和多样化 ✨
- **文件管理模块**: 多格式输出支持
- **基础测试**: 单元测试和集成测试

### ⏳ 开发中
- **混合检索器**: 向量和BM25结果融合
- **配置系统**: 模型和参数配置
- **Web部署**: Flask API接口

### 📋 待开发
- **问答系统**: 基于检索的智能问答
- **用户界面**: Web前端界面
- **性能优化**: 大规模数据处理
- **多模态融合**: 视频内容理解

## 🤝 贡献指南

1. Fork 项目
2. 创建功能分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

### 开发规范
- 遵循PEP 8代码风格
- 添加详细的文档字符串
- 编写单元测试
- 更新相关文档

## 📄 许可证

本项目采用 MIT 许可证 - 查看 [LICENSE](LICENSE) 文件了解详情

## 🙏 致谢

- [OpenAI Whisper](https://github.com/openai/whisper) - 语音识别
- [Sentence-Transformers](https://github.com/UKPLab/sentence-transformers) - 文本向量化
- [FFmpeg](https://ffmpeg.org/) - 音视频处理
- [HuggingFace Transformers](https://github.com/huggingface/transformers) - 预训练模型
- [SciPy](https://scipy.org/) - 科学计算库

## 📞 联系方式

如有问题或建议，请通过以下方式联系：

- 提交 [Issue](https://github.com/1625353276/Video_Assistant/issues)
- 发送邮件至: [2581489762@qq.com]

---

<div align="center">
  <p>如果这个项目对您有帮助，请给它一个 ⭐️</p>
</div>